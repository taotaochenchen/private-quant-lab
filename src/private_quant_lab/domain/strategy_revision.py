...
def build_decision_context(previous_report, previous_trade_date, trade_date, execution_feedback=""):
    """验证日期关系；上一交易日由调用方确认，未接交易日历，不用自然日减一。"""
    ...
    if previous_report is not None:
        ...
        if not isinstance(previous_trade_date, str) or not previous_trade_date:
    ...
    return {
        ...
        "calendar_source": "user_confirmed" if previous is not None else "unavailable",
